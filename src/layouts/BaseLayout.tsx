...
const NavContainer: React.FC<{ setNavOpen: React.Dispatch<React.SetStateAction<boolean>> }> = (props) => {
  ...
  return (
    <View className="w-[95%] bg-green-500 rounded-md mx-auto mt-[80px] z-30 p-5">
      ...
      <Pressable className="items-center py-3" onPress={() => handleRedirect("USER_PROFILE_SCREEN")}>
  ...
  return (
    <View className="flex-1">
      ...
      <View className="flex-1 z-20 relative">
        {navOpen && !props.hideNav ? <NavContainer setNavOpen={setNavOpen} /> : null}
